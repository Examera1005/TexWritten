import type { DetectedLanguage, OcrBlock, OcrResult } from "./types.js";
import { buildFullLatex } from "./formatter.js";
...
export function buildHeuristicResult(rawText: string, confidence: number, warnings: string[]): OcrResult {
  ...
  if (!normalized) {
    const blocks: OcrBlock[] = [
      {
        type: "unknown",
        raw_text: "[illegible]",
        latex: "% TODO: verify this formula\n[illegible]",
        confidence: 0,
        needs_review: true
      }
    ];
    ...
    return {
      ...
      warnings: ["No readable text extracted.", ...warnings]
    };
  }
